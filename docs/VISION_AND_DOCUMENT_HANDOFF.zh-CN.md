...
> **已失效（历史记录）**：`docs/adr/0002-narrow-to-credential-provider.md` 决定把本插件
> 收缩为凭据提供器，**整条视觉链路已随适配器一并删除**。本文描述的代码
> 已不在仓库中，保留它只是为了记录当时为何那样设计。**不要照本文实现。**
> 现在图片由 Harness 的 Copilot 路由处理，与本插件无关。
